...
def build_task_with_session(
    ...
    return (
        ...
        f"{params_text}"
        f"{feedback_text}"
        f"This is the AVIARY session_id -- use it for aviary tools "
        f"(set_aircraft_parameters, configure_mission, run_simulation, get_results, "
        f"check_constraints). Session setup for aviary is done.\n"
        f"OTHER SERVERS HAVE THEIR OWN SESSIONS: su2, tigl, pycycle and mass each keep a "
        f"separate namespace, so this id is NOT valid for them. Call get_design_state to "
        f"see the session id for every server, or create one with that server's own call "
        f"(create_su2_session, open_cpacs, create_cycle_model).\n"
        f"IF YOU ARE UNSURE WHETHER SOMETHING IS POSSIBLE, LOOK FOR A TOOL FIRST rather "
        f"than guessing a value: get_design_state (current sessions and parameters), "
